...
#define WLAYERS5 1 //表示等待来自上层的调用
#define WNACK 0    //等待ACK或NAK
...
void A_output(struct msg message)
{
    if (A.state == WLAYERS5)
    {
        printf("  A_output 成功发送包,并开始记时: %s\n", message.data);
        struct pkt sndpkt;
        sndpkt.seqnum = A.acknum;
        memmove(sndpkt.payload, message.data, 20);
        sndpkt.checksum = cal_csum(sndpkt);
        A.last_packet = sndpkt;
        A.state = WNACK;
        tolayer3(0, sndpkt);
...
void A_input(struct pkt packet)
{
    if (A.state != WNACK)
    {
        printf("  A_input 正在等待ACK或者NAK\n");
    }
    else if (packet.checksum != cal_csum(packet))
    {
        printf("  A_input corrupt错误:%s\n", A.last_packet.payload);
    }
    else if (packet.acknum != A.acknum)
    {
        printf("  A_input 正在等待ACK:%d , 而不是 %d\n", A.acknum, packet.acknum);
    }
    else
    {
        A.state = WLAYERS5;
        A.acknum = 1 - A.acknum;
        printf("  A_input 接到ACK,解除停等,等待上层调用\n");
        stoptimer(0);  
    }
    return;
}
...
//     if (A.state == WNACK)
//     {
//         printf("  A_timerinterrupt timeout 将重新发送 并开始计时: %s.\n", A.last_packet.payload);
//         tolayer3(0, A.last_packet);
//         starttimer(0, A.time_out);
//     }
//     else printf("  A_timerinterrupt 未在等待ACK, 不存在超时\n");
//     return;
// }
void A_timerinterrupt(void)
{
    if (A.state != WNACK)
    {
        printf("  A_timerinterrupt 未在等待ACK, 不存在超时\n");
        return;
    }
    printf("  A_timerinterrupt timeout 将重新发送 并开始计时: %s.\n", A.last_packet.payload);
    tolayer3(0, A.last_packet);
...
void ACK01(int AorB, int acknum)
{
    struct pkt newpkt;
    newpkt.acknum = acknum;
    newpkt.checksum = cal_csum(newpkt);
    tolayer3(AorB, newpkt);
    return;
}
...
void B_input(struct pkt packet)
{
    ...
    {
        printf("  B_input 包比特受损 将发送NCK\n");
        ACK01(1, 1 - B.seqnum);
        return;
    }
    else if (packet.seqnum != B.seqnum)
    {
        printf("  B_input 序号错误 将发送NCK\n");
        ACK01(1, 1 - B.seqnum);
        return;
    }
    else
    {
        printf("  B_input 接收到包%s\n", packet.payload);
        printf("  B_input 发送ACK\n");
        ACK01(1, B.seqnum);
